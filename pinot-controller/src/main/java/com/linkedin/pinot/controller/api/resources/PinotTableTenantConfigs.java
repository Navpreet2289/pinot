...
public class PinotTableTenantConfigs {
  ...
  public SuccessResponse rebuildBrokerResource(
    ...
    if (TableNameBuilder.extractRawTableName(tableNameWithType).equals(tableNameWithType)) {
      throw new ControllerApplicationException(LOGGER, "Table name has to be with table type.", Response.Status.BAD_REQUEST);
    }
    try {
